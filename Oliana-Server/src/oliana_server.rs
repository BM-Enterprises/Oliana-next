...
async fn main_async() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut procs = oliana_lib::launchers::TrackedProcs::new(expected_bin_directory, track_proc_dir.clone());
    ...
    tokio::join!(
        ...
        ipv6_listener
            ...
            // the generated World trait.
            .map(|channel| {
                let server = oliana_server_lib::OlianaServer::new( channel.transport().peer_addr().expect("IPv6 Client had no peer_addr!"), shareable_procs.clone() );
                channel.execute(server.serve()).for_each(spawn)
            })
            // Max 10 channels.
            .buffer_unordered(10)
